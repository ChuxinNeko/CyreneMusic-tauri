...
interface DefaultBackgroundBounds {
    left: number;
    top: number;
    right: number;
    bottom: number;
...
type DefaultBackgroundShapeKind = 'ring' | 'square' | 'cross' | 'spark';
type DefaultBackgroundAudioBand = 'bass' | 'lowMid' | 'mid' | 'vocal' | 'treble';
type DefaultBackgroundShapeColor = 'secondary' | 'accent';

interface DefaultBackgroundShape {
    id: string;
    kind: DefaultBackgroundShapeKind;
    x: number;
    y: number;
    width: number;
    height: number;
    ...
    strokeWidth: number;
    opacity: number;
    color: DefaultBackgroundShapeColor;
    depth: number;
    audioBand?: DefaultBackgroundAudioBand;
    ringGapStart?: number;
    ringGapSize?: number;
}

export interface DefaultBackgroundScene {
    width: number;
    height: number;
    shapes: DefaultBackgroundShape[];
}

export type DefaultBackgroundAudioLevels = Partial<Record<DefaultBackgroundAudioBand, number>> & {
    power?: number;
};

interface DefaultBackgroundParallax {
    cameraX: number;
    cameraY: number;
    originX: number;
    originY: number;
...
const normalizeBounds = (
    bounds: DefaultBackgroundBounds,
    worldWidth: number,
    worldHeight: number,
): DefaultBackgroundBounds => ({
...
const choosePaperHaloAnchor = ({
    ...
    paperBounds: DefaultBackgroundBounds;
    worldWidth: number;
    worldHeight: number;
    width: number;
    height: number;
...
export const buildDefaultBackgroundScene = ({
    viewport,
    world,
    paperBounds,
    seed,
    ...
    const shapeKinds: DefaultBackgroundShapeKind[] = ['ring', 'square', 'cross', 'ring', 'square', 'cross'];
    const shapeCount = worldWidth > worldHeight ? 8 : 7;
    const sparkBands: DefaultBackgroundAudioBand[] = ['treble', 'vocal', 'mid', 'treble', 'lowMid'];
    ...
    const baseShapes = Array.from({ length: shapeCount }).map((_, index) => {
        const localSeed = `${seed ?? 'default'}:${worldWidth}:${worldHeight}:${index}`;
        ...
        return {
            ...
            color: (seeded(`${localSeed}:color`) > 0.5 ? 'accent' : 'secondary') as DefaultBackgroundShapeColor,
    ...
    const sparkShapes = Array.from({ length: sparkCount }).map((_, index) => {
        const localSeed = `${seed ?? 'default'}:${worldWidth}:${worldHeight}:spark:${index}`;
...
export const drawDefaultBackground = ({
    context,
    scene,
    theme,
    time = 0,
    ...
    context: CanvasRenderingContext2D;
    scene: DefaultBackgroundScene;
    theme: Theme;
    time?: number;
    audioLevels?: DefaultBackgroundAudioLevels;
    parallax?: DefaultBackgroundParallax;
