...
// Per-word color resolution cache — word text + theme accent color → resolved color.
// Cleared on theme change via clearColorCache.
const _wordColorCache = new Map<string, string>();
const WORD_COLOR_CACHE_MAX = 256;

/** Clear the word color resolution cache (call on theme change). */
export const clearWordColorCache = () => { _wordColorCache.clear(); };
...
export const resolveWordColor = (
    ...
    // Cache lookup: same word + same fallback color → same result within a theme session.
    const cacheKey = `${cleanCurrent}\x00${fallbackColor}\x00${cjkMatchMode}`;
    const cached = _wordColorCache.get(cacheKey);
    ...
    const result = resolveWordColorEntryColor(matched) || fallbackColor;
    if (_wordColorCache.size >= WORD_COLOR_CACHE_MAX) _wordColorCache.clear();
    _wordColorCache.set(cacheKey, result);
    return result;
};
