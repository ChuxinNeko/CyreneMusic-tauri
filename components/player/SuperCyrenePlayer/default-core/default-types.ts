import type { LineRenderHints } from './renderHints';
...
export type BuiltinVisualizerMode = 'classic' | 'cadenza' | 'partita' | 'default' | 'monet';
...
export interface DefaultTuning {
...
export const DEFAULT_DEFAULT_TUNING: DefaultTuning = {
...
export type ChatEmojiPackSource = 'builtin' | 'custom';
export type ChatAvatarSource = 'cover' | 'builtin' | 'color' | 'custom';

export interface ChatTuning {
  showEmoMessages: boolean;
  emojiPackSource: ChatEmojiPackSource;
  avatarSource: ChatAvatarSource;
}

export const DEFAULT_CHAT_TUNING: ChatTuning = {
...
export interface StoredChatEmojiImage {
  id: string;
  name: string;
  mimeType: string;
  blob: Blob;
...
export interface ChatEmojiImage {
  id: string;
  name: string;
  url: string;
}

export interface StoredChatAvatarImage {
  id: string;
  name: string;
  mimeType: string;
  blob: Blob;
...
export interface ChatAvatarImage {
  id: string;
  name: string;
  url: string;
}
...
// Local-library contracts are outside the renderer boundary and intentionally omitted.
